...
installed somewhere `src/` cannot be reached:
    ...
    uv run --no-project --with dist/*.whl python scripts/smoke_test_wheel.py 0.2.1

Python puts this file's own directory on `sys.path` rather than the working
directory, so `import cableloss` below can only resolve to the installed wheel.
"""
...
def main(expected: str) -> None:
    installed = importlib.metadata.version("cableloss")
    if installed != expected:
        raise SystemExit(f"The wheel installed {installed}, expected {expected}")

    for name in cableloss.__all__:
        getattr(cableloss, name)

    if not importlib.resources.files("cableloss").joinpath("py.typed").is_file():
        raise SystemExit("The wheel is missing py.typed")

    print(f"cableloss {installed} imported from {cableloss.__file__}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check a built cableloss wheel from outside the source tree."
    )
    parser.add_argument(
